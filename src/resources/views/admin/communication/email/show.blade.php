...
<style>
    ...
    .email-show-form-label { font-size: 0.775rem; color: var(--text-secondary, #8c8c8c); }
</style>
@endpush
            ...
            <div class="page-header">
                    ...
                    <div class="breadcrumb-wrapper">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item">
                                    <a href="{{ route("admin.communication.email.index") }}">{{ translate("Email Dispatch Logs") }}</a>
                                </li>
                ...
                <div class="page-header-right d-flex gap-2">
                    <button class="i-btn btn--primary btn--sm resend-email-log"
                            data-url="{{ route('admin.communication.email.resend', $log->id) }}">
                        <i class="ri-mail-send-line"></i> {{ translate("Resend") }}
                    </button>
                    <a href="{{ route('admin.communication.email.index') }}" class="i-btn btn--dark outline btn--sm">
            ...
            <div class="row g-4">
                    ...
                    <div class="card email-show-card">
                        ...
                        <div class="email-show-header">
                            <div class="d-flex align-items-start justify-content-between gap-3">
                                ...
                                <div class="d-flex align-items-center gap-2 flex-shrink-0">
                                    ...
                                    <a href="{{ route('admin.communication.email.show', $log->id) }}?raw=1" target="_blank" class="d-flex align-items-center text-decoration-none" style="font-size: 0.9rem; color: var(--color-primary-text); opacity: 0.85;" title="{{ translate('Open in New Tab') }}">
                        ...
                        <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 email-show-strip">
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <div class="lh-sm">
                                    <span class="fw-semibold">{{ $emailFromName ?? translate("N/A") }}</span>
                                    @if($emailFrom)
                        ...
                        <div class="email-show-strip">
                            ...
                            @foreach($attachments as $attachment)
                                @php
                                    ...
                                    $viewUrl = $storedName ? route('admin.communication.email.attachment.download', [$log->id, $storedName]) . '?view=1' : null;
                                    $downloadUrl = $storedName ? route('admin.communication.email.attachment.download', [$log->id, $storedName]) : null;
                                @endphp
                                @if($storedName)
                                    @if($isImage)
                ...
                <div class="col-lg-4">
                        ...
                        <div class="card-body" style="padding: 20px;">
                            ...
                            <div class="d-flex flex-column gap-3">
                                ...
                                <div class="d-flex align-items-center justify-content-between">
                                    ...
                                    <span class="email-show-detail-value">{{ $log->gatewayable->name }}</span>
                                </div>
                                <div class="d-flex align-items-center justify-content-between">
                                ...
                                @if($log->user)
                                <div class="d-flex align-items-center justify-content-between">
                                    <span class="email-show-detail-label">{{ translate("User") }}</span>
                                    <a href="{{ route('admin.user.details', $log->user->id) }}" class="email-show-detail-value" style="color: var(--color-primary);">{{ $log->user->name }}</a>
                                </div>
                                @endif
                                @if($log->campaign)
                                <div class="d-flex align-items-center justify-content-between">
                    ...
                    <div class="card email-show-sidebar-card">
                        <div class="card-body" style="padding: 20px;">
                            <h6 class="fw-semibold mb-3 email-show-sidebar-title">{{ translate("Update Status") }}</h6>
                            <form action="{{ route('admin.communication.email.status.update') }}" method="POST">
                                @csrf
                                <input type="hidden" name="id" value="{{ $log->id }}">
                                <div class="d-flex flex-column gap-3">
                                    <div>
                                        <label for="status" class="form-label mb-1 email-show-form-label">{{ translate("Status") }}</label>
                                        <select data-placeholder="{{ translate('Select A Delivery Status') }}" class="form-select form-select-sm select2-search" name="status" id="status">
                                            <option value=""></option>
                                            @foreach(\App\Enums\System\CommunicationStatusEnum::getValues() as $value)
                                                @if(!in_array($value, [\App\Enums\System\CommunicationStatusEnum::SCHEDULE->value, \App\Enums\System\CommunicationStatusEnum::PENDING->value]))
                                                    <option value="{{ $value }}" {{ $log->status->value == $value ? 'selected' : '' }}>{{ ucfirst(strtolower($value)) }}</option>
                                                @endif
                                            @endforeach
                                        </select>
                                    </div>
                                    <div id="gateway-container" style="{{ $log->status->value == \App\Enums\System\CommunicationStatusEnum::PENDING->value ? '' : 'display:none;' }}">
                                        <label for="gateway_id" class="form-label mb-1 email-show-form-label">{{ translate("Gateway") }}</label>
                                        <select class="form-select form-select-sm select2-search" id="gateway_id" data-placeholder="{{ translate("Select a gateway") }}" name="gateway_id">
                                    ...
                                    <div id="fail-reason-container" style="{{ $log->status->value == \App\Enums\System\CommunicationStatusEnum::FAIL->value ? '' : 'display:none;' }}">
                                        <label for="fail_reason" class="form-label mb-1 email-show-form-label">{{ translate("Failure Reason") }}</label>
                                        <input type="text" class="form-control form-control-sm" name="response_message" id="fail_reason" placeholder="{{ translate('Enter reason for failure') }}" value="{{ $log->response_message }}">
                                    </div>
                                    <button type="submit" class="i-btn btn--primary btn--sm w-100">
                                        <i class="ri-save-line"></i> {{ translate("Update Status") }}
                                    </button>
                                </div>
                            </form>
                        </div>
    ...
    $(document).ready(function () {
        $('#status').on('change', function () {
            ...
            if (val === 'fail') {
                ...
                $('#gateway-container').hide();
            } else if (val === '{{ \App\Enums\System\CommunicationStatusEnum::PENDING->value }}') {
                $('#fail-reason-container').hide();
                $('#gateway-container').show();
            } else {
                $('#fail-reason-container').hide();
                $('#gateway-container').hide();
            }
