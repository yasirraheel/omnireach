                                    ...
                                    <tr class="@if($loop->even)@endif">
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center gap-2 ">
                                                <div class="lh-1">
                                                    <p class="fs-12 fw-medium mb-2">{{ translate('First Name: ') }} {{ $unsubscription->contact?->first_name ?? translate('N/A') }}</p>
                                                    @if($type == (string)\App\Enums\ServiceType::EMAIL->value) 
                                                        <p class="fs-12 fw-medium mb-2">{{ translate('Email Address: ') }} {{ $unsubscription->contact?->email_contact ?? translate('N/A') }}</p>
                                                    @endif
                                                    @if($type == (string)\App\Enums\ServiceType::WHATSAPP->value) 
                                                        <p class="fs-12 fw-medium mb-2">{{ translate('WhatsApp Contact: ') }} {{ $unsubscription->contact?->whatsapp_contact ?? translate('N/A') }}</p>
                                                    @endif
                                                    @if($type == (string)\App\Enums\ServiceType::SMS->value) 
                                                        <p class="fs-12 fw-medium mb-2">{{ translate('SMS Contact: ') }} {{ $unsubscription->contact?->sms_contact ?? translate('N/A') }}</p>
                                                    @endif
                                                </div>
                                            </div>
                                        </td>
