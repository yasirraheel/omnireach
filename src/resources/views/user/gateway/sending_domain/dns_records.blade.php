                ...
                <div class="breadcrumb-wrapper">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item">
                                <a href="{{ route("user.gateway.sending-domain.index") }}">{{ translate("Sending Domains") }}</a>
                            </li>
                    ...
                    <div class="card-body">
                        <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
                            ...
                            <div class="d-flex gap-2">
                                @if($keysGenerated)
                                    <button type="button" class="i-btn btn--primary btn--sm" id="verifyDnsBtn" data-url="{{ route('user.gateway.sending-domain.verify', $domain->uid) }}">
        ...
        @if(!$keysGenerated)
            ...
            <div class="alert alert-danger d-flex align-items-start gap-3 mb-4" role="alert">
                ...
                <div>
                    ...
                    <form action="{{ route('user.gateway.sending-domain.regenerate', $domain->uid) }}" method="POST" class="d-inline">
                        @csrf
            ...
            @if($opensslCheck && !$opensslCheck['ready'])
                <div class="alert alert-warning d-flex align-items-start gap-3 mb-4" role="alert">
                    <i class="ri-information-line fs-4 mt-1"></i>
                    <div>
                        <strong>{{ translate('Server Configuration Issue') }}</strong>
                        <p class="mb-0 mt-1">{{ translate("DKIM key generation requires the OpenSSL PHP extension. Please contact your administrator to ensure OpenSSL is properly configured on the server.") }}</p>
                    </div>
                </div>
            @endif
        @else
...
<div class="modal fade actionModal" id="regenerateKeysModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="{{ route('user.gateway.sending-domain.regenerate', $domain->uid) }}" method="POST">
                @csrf
