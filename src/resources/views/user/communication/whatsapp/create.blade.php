...
if (SpeechRecognition && speechElements.length > 0) {
  ...
  $('#text-to-speech-icon').on('click', function() {
      ...
      // Show template preview with message, media, and buttons
      function showTemplatePreview(templateData) {
        var $previewSection = $('.whatsapp-template-preview');
        var $mediaPreview = $('.template-media-preview');
        var $messagePreview = $('.template-message-text');
        var $buttonsPreview = $('.template-buttons-preview');
        var $buttonsList = $('.template-buttons-list');

        // Reset all previews
        $mediaPreview.addClass('d-none');
        $buttonsPreview.addClass('d-none');
        $buttonsList.empty();

        // Show message preview
        var message = templateData.message || '';
        $messagePreview.text(message);
        $previewSection.removeClass('d-none');

        // Show media preview if available
        var mediaUrl = templateData.image_url || templateData.media_url || templateData.imageUrl || '';
        if (mediaUrl) {
          $('.template-media-url').text(mediaUrl);
          $mediaPreview.removeClass('d-none');
        }

        // Show buttons preview if available
        var buttons = templateData.buttons || [];
        if (buttons.length > 0) {
          buttons.forEach(function(button, index) {
            var buttonText = typeof button === 'string' ? button : (button.text || button.title || button.buttonText || '');
            if (buttonText) {
              $buttonsList.append(
          ...
          $buttonsPreview.removeClass('d-none');
        }
      }
      ...
      $('.available-whatsapp-template').on('click', function() {
        ...
        $('#availableWhatsappTemplate').modal('show');
      });
      ...
      // Apply template to message area
      $('#saveWhatsappTemplateButton').on('click', function() {
        ...
        // Build the full message including buttons as numbered options
        var fullMessage = currentTemplateData.message || '';

        // Add buttons as numbered text options
        var buttons = currentTemplateData.buttons || [];
        if (buttons.length > 0) {
        ...
        // Set the message in textarea
        var $messageTextarea = $('#message');
        $messageTextarea.val(fullMessage);

        // Handle media URL - store in hidden input
        var mediaUrl = currentTemplateData.image_url || currentTemplateData.media_url || currentTemplateData.imageUrl || '';
        if (mediaUrl) {
          // Create or update hidden input for media URL
          var $mediaInput = $('input[name="message[media_url]"]');
          if ($mediaInput.length === 0) {
            ...
            $mediaInput.val(mediaUrl);
          }

          // Show visual feedback for media attachment
          var $mediaPreviewDiv = $('#add_media');
          $mediaPreviewDiv.removeClass('d-none').html(
            '<div class="alert alert-info d-flex align-items-center gap-2 py-2 mb-0">' +
              '<i class="ri-image-line fs-5"></i>' +
              '<div>' +
                '<strong>{{ translate("Media from template") }}</strong>' +
                '<p class="mb-0 small text-truncate" style="max-width: 300px;">' + mediaUrl + '</p>' +
              '</div>' +
        ...
        // Store buttons data in hidden input for processing
        if (buttons.length > 0) {
          var $buttonsInput = $('input[name="message[buttons]"]');
          if ($buttonsInput.length === 0) {
            ...
            $buttonsInput.val(JSON.stringify(buttons));
          }
        }
      ...
      $('#availableWhatsappTemplate').on('hidden.bs.modal', function() {
        // Don't reset if template was successfully applied
      });

    });

    // Global function to remove template media
    function removeTemplateMedia() {
