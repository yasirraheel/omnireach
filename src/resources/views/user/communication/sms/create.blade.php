                          ...
                          <div class="form-inner">
                            ...
                            <select class="form-select select2-search" id="chooseMethod" data-placeholder="{{ translate("Choose a sending method") }}" aria-label="chooseMethod" name="method">
                              ...
                              @if(array_key_exists('is_allowed', $planAccess->android) && $planAccess->android['is_allowed'])
                                <option value="{{ \App\Enums\System\Gateway\SmsGatewayTypeEnum::ANDROID->value }}" {{ old('method') == \App\Enums\System\Gateway\SmsGatewayTypeEnum::ANDROID->value ? 'selected' : '' }}>{{ translate("Android Gateway") }}</option>
                              @endif
                              @if(array_key_exists('is_allowed', $planAccess->sms) && $planAccess->sms['is_allowed'])
                                <option value="{{ \App\Enums\System\Gateway\SmsGatewayTypeEnum::API->value }}" {{ old('method') == \App\Enums\System\Gateway\SmsGatewayTypeEnum::API->value ? 'selected' : '' }}>{{ translate("API Gateway") }}</option>
                              @endif
                            </select>
                          </div>
                        </div>
