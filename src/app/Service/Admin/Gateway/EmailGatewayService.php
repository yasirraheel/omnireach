...
class EmailGatewayService
{
    ...
    public function param($request, $user_id) {
        ...
        $data = [
            ...
            'per_message_min_delay'  => $request->input('per_message_min_delay', 0),
            'per_message_max_delay'  => $request->input('per_message_max_delay', 0),
            'delay_after_count'      => $request->input('delay_after_count', 0),
            'delay_after_duration'   => $request->input('delay_after_duration', 0),
            'reset_after_count'      => $request->input('reset_after_count', 0),
        ];

        if($request->has('bulk_contact_limit')) {
            $data['bulk_contact_limit'] = $request->input('bulk_contact_limit', 1);
        }
    ...
    public function gatewayTest($gateway, $email_to) {
        ...
        if ($response==null) {
            ...
            $data = json_encode([
                'message' => translate('Test email sent successfully to ') . $email_to,
                'status'  => true,
            ]);
        }
        else{
            $data = json_encode([
                'message' => translate('Failed to send test email via ') . $gateway->name . ($response && is_string($response) ? ': ' . $response : ''),
                'status'  => false,
            ]);
        }
