...
class CampaignController extends Controller
{
    ...
    public function create($channel) {
        ...
        if ($channel == Campaign::EMAIL) {
            ...
            if($allowed_access->type == StatusEnum::FALSE->status() && $user->gateways->isNotEmpty() && $user->gateways()->mail()->active()->exists()) {
            ...
            if(($allowed_access->type == StatusEnum::FALSE->status() && $user->gateways->isNotEmpty() && $user->gateways()->sms()->active()->exists()) || auth()->user()->sms_gateway == 2) {
