   ...
   /**
    ...
    public function whatsappQRGenerate(Request $request): bool|string {
        ...
        try {
            ...
            if($whatsapp_access->is_allowed && ($whatsapp_device_count < $whatsapp_access->gateway_limit || $whatsapp_access->gateway_limit == -1)) { 
                ...
                if ($response->status() === 200) {
                    ...
                    if ($response->status() === 500) {
                        $msg = translate("WhatsApp service is temporarily unavailable. Please contact your administrator for assistance.");
                    } elseif ($response->status() === 503 || $response->status() === 502) {
                        $msg = translate("WhatsApp service is currently down. Please contact your administrator for assistance.");
                    } else {
                        $msg = translate("Unable to connect to WhatsApp service. Please contact your administrator for assistance.");
                    }
            ...
            $data['status']  = 500;
            $data['qr']      = '';
            $data['message'] = translate("WhatsApp service is currently unavailable. Please contact your administrator for assistance.");
