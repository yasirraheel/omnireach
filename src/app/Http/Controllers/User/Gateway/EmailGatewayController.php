    ...
    public function testGateway(Request $request) :mixed {
        ...
        try {
            ...
            if($request->input('gateway_id')) {
                $mailGateway = Gateway::mail()
                                    ->where('id', $request->input('gateway_id'))
                ...
                $mailGateway = Gateway::mail()->where('user_id', $user->id)->where('is_default', 1)->first();

                if(!$mailGateway) {
                    $mailGateway = Gateway::mail()->whereNull('user_id')->where('is_default', 1)->first();
                }
            }
            ...
            if($mailGateway == null) {
                ...
                return json_encode([
                    'message' => translate('No mail gateway found'),
                    'status'  => false,
                ]);
            }
