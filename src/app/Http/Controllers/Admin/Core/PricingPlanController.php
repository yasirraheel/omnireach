...
class PricingPlanController extends Controller
{
    ...
    public function bulk(Request $request): RedirectResponse
    {
        ...
        try {
            ...
            // Clear pricing plans cache so frontend reflects changes immediately
    ...
    public function create(): View
    {
        ...
        $android_gateways = AndroidSession::whereNull("user_id")
                                                ...
                                                ->pluck('name', 'id')
                                                ->toArray();
        $displayFeatures = PlanDisplayFeature::active()->ordered()->get();
        return view('admin.membership.plan.create', compact('title', 'sms_credentials', 'mail_credentials', 'sms_gateways', 'android_gateways', 'displayFeatures'));
    }
    ...
    public function store(Request $request)
    {
        try {
            $validations = [
                ...
                'affiliate_commission' => 'required|numeric|gte:0',
            ...
            // Add domain limits to email config
            $planMapping['email']['max_sending_domains'] = (int) ($request->input('max_sending_domains') ?? 3);
            $planMapping['email']['max_tracking_domains'] = (int) ($request->input('max_tracking_domains') ?? 2);

            // Handle Lead Generation settings
            $data['lead_generation'] = [
                'is_allowed' => (bool) $request->input('lead_generation_enabled'),
                'daily_limit' => (int) ($request->input('lead_daily_limit') ?? 100),
                'monthly_limit' => (int) ($request->input('lead_monthly_limit') ?? 1000),
            ];

            // Handle Workflow Automation settings
            $data['automation'] = [
                'is_allowed' => (bool) $request->input('automation_enabled'),
                'workflow_limit' => (int) ($request->input('automation_workflow_limit') ?? 5),
                'execution_limit' => (int) ($request->input('automation_execution_limit') ?? 1000),
            ];

            // Handle AI Campaign Intelligence settings
            $data['ai_intelligence'] = [
                'is_allowed' => (bool) $request->input('ai_intelligence_enabled'),
                'campaign_insights' => (bool) $request->input('ai_campaign_insights'),
                'ab_testing' => (bool) $request->input('ai_ab_testing'),
                'ab_test_limit' => (int) ($request->input('ai_ab_test_limit') ?? 0),
                'send_time_optimizer' => (bool) $request->input('ai_send_time_optimizer'),
            ];
            ...
            // Sync display features (always sync to set is_included for all features)
            $displayFeatureIds = $request->input('display_features', []);
            $plan->syncDisplayFeatures(array_map('intval', $displayFeatureIds));
    ...
    public function edit($id)
    {
        ...
        $displayFeatures  = PlanDisplayFeature::active()->ordered()->get();
        $assignedFeatureIds = $plan->displayFeatures()->wherePivot('is_included', true)->pluck('plan_display_feature_id')->toArray();

        return view('admin.membership.plan.edit', compact('title', 'sms_credentials', 'mail_credentials', 'plan', 'sms_gateways', 'mail_gateways', 'displayFeatures', 'assignedFeatureIds'));
    }
    ...
    public function update(PricingPlanRequest $request) {
        ...
        try {
            ...
            // Capture old plan data before update for smart device limit comparison
            $oldPlanData = $plan->replicate();
            $oldPlanData->id = $plan->id; // Preserve ID for comparison
            ...
            // Smart device management - only deactivate excess devices if limits reduced
            $this->pricingPlanService->updatePlanRelatedModels($plan->id, $oldPlanData);
     ...
     */
    public function subscribersCount($id)
    {
        $count = Subscription::where('plan_id', $id)
    ...
    /**
     * Sync plan updates to active subscribers
     * This updates feature access (automatic via planAccess) and optionally syncs credit limits
     */
    public function syncSubscribers(Request $request, $id)
    {
        try {
            $plan = PricingPlan::findOrFail($id);
            $syncCredits = $request->input('sync_credits', false);
            ...
            if ($subscriptions->isEmpty()) {
                return response()->json([
                    'success' => false,
                    'message' => translate('No active subscribers found for this plan.')
                ]);
            }
            ...
            foreach ($subscriptions as $subscription) {
                ...
                try {
                    // Sync credits if requested
                    // IMPORTANT: We only ADD credits if the new plan has MORE credits
                    // We don't reduce credits to protect admin revenue
                    if ($syncCredits) {
                        $this->syncUserCredits($user, $plan);
                    }

                    // Features (lead_generation, automation, ai_intelligence) are automatically
                    // synced because planAccess() reads from the live PricingPlan model
            ...
            $message = translate('Successfully synced') . ' ' . $syncedCount . ' ' . translate('subscribers.');
            if (!empty($errors)) {
                $message .= ' ' . count($errors) . ' ' . translate('errors occurred.');
                \Log::warning('Plan sync errors', ['plan_id' => $id, 'errors' => $errors]);
            }
            ...
            return response()->json([
                'success' => false,
                'message' => translate('Sync failed: ') . $e->getMessage()
            ], 500);
        }
    }

    /**
     * Sync credits for a user based on plan
     * Only increases credits, never decreases (to protect admin revenue)
     */
    private function syncUserCredits(User $user, PricingPlan $plan): void
    {
        $updated = false;

        // SMS Credits
        $planSmsCredits = $plan->sms->credits ?? 0;
        if ($planSmsCredits == -1) {
            // Unlimited - set to -1
            $user->sms_credit = -1;
            $updated = true;
        } elseif ($planSmsCredits > 0 && $user->sms_credit != -1) {
            // Only increase if plan has more credits than user currently has
            // This ensures we don't reset used credits
            if ($user->sms_credit < $planSmsCredits) {
                $user->sms_credit = $planSmsCredits;
                $updated = true;
            }
        }

        // Email Credits
        $planEmailCredits = $plan->email->credits ?? 0;
        if ($planEmailCredits == -1) {
            $user->email_credit = -1;
            $updated = true;
        } elseif ($planEmailCredits > 0 && $user->email_credit != -1) {
            if ($user->email_credit < $planEmailCredits) {
                $user->email_credit = $planEmailCredits;
                $updated = true;
            }
        }

        // WhatsApp Credits
        $planWhatsappCredits = $plan->whatsapp->credits ?? 0;
        if ($planWhatsappCredits == -1) {
            $user->whatsapp_credit = -1;
            $updated = true;
        } elseif ($planWhatsappCredits > 0 && $user->whatsapp_credit != -1) {
            if ($user->whatsapp_credit < $planWhatsappCredits) {
                $user->whatsapp_credit = $planWhatsappCredits;
                $updated = true;
            }
        }
    ...
    /**
     * Get subscribers list for a plan (for modal display)
     */
    public function subscribers($id)
    {
        $plan = PricingPlan::findOrFail($id);

        $subscribers = Subscription::where('plan_id', $id)
            ->whereIn('status', [Subscription::RUNNING, Subscription::RENEWED])
            ->with('user:id,name,email,sms_credit,email_credit,whatsapp_credit')
        ...
        return response()->json([
            'plan' => [
                'id' => $plan->id,
                'name' => $plan->name,
                'sms_credits' => $plan->sms->credits ?? 0,
                'email_credits' => $plan->email->credits ?? 0,
                'whatsapp_credits' => $plan->whatsapp->credits ?? 0,
            ],
