...
class CommunicationController extends Controller
{
    ...
    public function api(Request $request) {
        ...
        $gateways = $this->getApiGateways();

        return view('admin.communication.api', compact('title', 'api_key', 'gateways'));
    }

    /**
     * Get available gateways for API settings (Admin)
     *
     * @return array
     */
    protected function getApiGateways(): array {

        // WhatsApp gateways (admin-level, no user_id) - Gateway uses Status::ACTIVE ('active')
    ...
    /**
     * Save API gateway preferences (Admin)
     *
     * @param Request $request
     * @return RedirectResponse
     */
    public function apiGatewaySave(Request $request): RedirectResponse {

        $status = 'error';
        $message = translate("Something went wrong");

        try {
            $data = $request->input('site_settings', []);
            ...
            // Clear the site settings cache so changes take effect immediately
            Cache::forget('site_settings');

            $status = 'success';
            $message = translate("API Gateway preferences saved successfully");
